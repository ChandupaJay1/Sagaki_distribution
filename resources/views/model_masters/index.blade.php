...
<div class="row g-4 mb-4">
    <div class="col-12 d-flex justify-content-between align-items-center">
        <div>
            <h4 class="fw-bold text-dark mb-1">Model Master</h4>
            <p class="text-muted small mb-0">Manage product models</p>
        </div>
        <a href="{{ route('model-masters.create') }}" class="btn btn-primary btn-sm rounded-pill"><i class="ri-add-line me-1"></i>Add Model</a>
    </div>
    </div>
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-light">
                    <tr>
                        ...
                        <th class="text-muted small fw-bold text-uppercase">Model Name</th>
                ...
                <tbody>
                    @forelse($models as $m)
                        <tr>
                            <td class="ps-4">
                                @if($m->code)
                                    <span class="badge bg-light text-dark border fw-medium px-2 py-1">{{ $m->code }}</span>
                                @else
                            ...
                            <td class="pe-4 text-end">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="{{ route('model-masters.edit', $m->id) }}" class="btn btn-sm btn-light border-0 rounded-circle text-primary" title="Edit">
                                        <i class="ri-edit-2-line fs-16"></i>
                                    </a>
                                    <form action="{{ route('model-masters.destroy', $m->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this model?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-5 text-muted">No models found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
